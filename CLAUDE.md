...
- **`server/`** — the Node process: Express on 3001, MongoDB, and two routes. Same split as above — pure logic with no database (`merge`, `passwords`) apart from what talks to Mongo (`db`, `identity`). It compiles with its own `tsconfig.server.json`, because the client one loads the DOM library and Vite's types. Relative imports there need a `.js` extension: the project is ESM and that tsconfig uses `NodeNext`.
